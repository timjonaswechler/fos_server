...
use {
    ...
    bevy_egui::{egui, EguiContexts, EguiPlugin, EguiPrimaryContextPass},
...
const INPUT_SEND_RATE: f32 = 120.0;
...
impl InputSendTimer {
    fn new(rate_hz: f32) -> Self {
        Self(Timer::from_seconds(1.0 / rate_hz, TimerMode::Repeating))
    }

    fn tick_and_should_send(&mut self, delta: std::time::Duration) -> bool {
        self.0.tick(delta);
        self.0.times_finished_this_tick() > 0
    }
}
...
fn main() -> AppExit {
    App::new()
        ...
        .insert_resource(InputSendTimer::new(INPUT_SEND_RATE))
...
fn handle_inputs(
    ...
    if !send_timer.tick_and_should_send(time.delta()) {
        return;
    }
